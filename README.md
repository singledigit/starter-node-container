...
```bash
starter-node-container$ sam build
```
...
```bash
starter-node-container$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
starter-node-container$ sam local start-api
starter-node-container$ curl http://localhost:3000/
```
...
```bash
starter-node-container$ sam logs -n HelloWorldFunction --stack-name starter-node-container --tail
```
...
```bash
starter-node-container$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name starter-node-container
```
